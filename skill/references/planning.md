...
| --- | --- | --- |
| **Explain** | New to this viewer, affects money or decisions, or easy to get wrong. The reason the screen exists. | Its own scene: two or three sentences, the camera zooms in, the narration box names it |
| **Mention** | Needed for context, but familiar or self-explanatory: filters, pickers, standard tables | One sentence, the narration box at most, usually no zoom |
...
- **Point at what you talk about, and only that** (signalling). The narration box on one thing at a time: what the current sentence names. Clear it when the narration moves on.
...
Verify backs these up with pacing warnings: camera moves too close together, scenes too long or too short, a narration box that lingers, and a video that zooms in before showing the whole screen.

## Placing cues and beats

A cue is a marker such as `[total]` written at the **start of a sentence**. A beat is an action tied to a cue: move the camera, move the narration box, or both. Two cues exist in every scene without being written: `start` (the scene's first frame) and `end` (when its narration finishes).
...
- **Put the narration box on what you name, and only what you name.** Clear it (`"highlight": false`) when the narration moves on to something general, so the dimmed screen does not linger. An `end` beat is a good place.
...
- **Each cue has at most one beat.** Put the camera and the narration box on the same beat.
