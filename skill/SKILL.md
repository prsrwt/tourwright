...
2. **Understand the screen and classify every part.** Read the screen's components and, where they exist, their tests and docs, then sort every section into **Explain**, **Mention** or **Skip** (see "Deciding what to explain"). Done when each part has a level and a one-line reason.
3. **Draft the storyboard, and get it approved. Do not write beats before this.** Show the user a table: for each scene, what it shows, why it matters to the viewer, its level, and roughly how many seconds. Wait for their answer and apply their corrections. This is where a misunderstanding of the product is cheapest to fix. Done when the user has approved it.
